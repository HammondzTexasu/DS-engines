...
Interpolace mezi klíčovými body (`start`, `end`, případně mezikroky) probíhá pomocí kubických Bézierových křivek. Stejný princip platí pro Tone (T), Hue (H) i Chroma (C). Hue interpolate volitelně `hueArc`: `shortest` (default) \| `longest` \| `linear` \| `increasing` \| `decreasing`.
